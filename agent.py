...
class PPOAgent(object):
    ...
    def step(self):
        ...
        for _ in range(hyperparameters['rollout_length']):
            actions, log_probs, _, values = self.network(torch.FloatTensor(states).to(self.config['pytorch']['device']))
        ...
        self.states = states
        pending_value = self.network(torch.FloatTensor(states).to(self.config['pytorch']['device']))[-1]
        rollout.append([states, pending_value, None, None, None, None])

        processed_rollout = [None] * (len(rollout) - 1)
        advantages = torch.FloatTensor(np.zeros((self.config['environment']['number_of_agents'], 1))).to(self.config['pytorch']['device'])
        returns = pending_value.detach()
        for i in reversed(range(len(rollout) - 1)):
            states, value, actions, log_probs, rewards, terminals = rollout[i]
            terminals = torch.FloatTensor(terminals).unsqueeze(1).to(self.config['pytorch']['device'])
            rewards = torch.FloatTensor(rewards).unsqueeze(1).to(self.config['pytorch']['device'])
            # rewards = torch.Tensor(rewards).unsqueeze(1)
            actions = actions
            states = torch.FloatTensor(states).to(self.config['pytorch']['device'])
        ...
        for _ in range(hyperparameters['optimization_epochs']):
            ...
            while not batcher.end():
                ...
                obj_clipped = torch.clamp(ratio, 1.0 - hyperparameters['ppo_clip'],
                                          1.0 + hyperparameters['ppo_clip']) * sampled_advantages
                policy_loss = -torch.min(obj, obj_clipped).mean(0) - hyperparameters['entropy_coefficent'] * entropy_loss.to(self.config['pytorch']['device']).mean()
